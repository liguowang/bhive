...
# -*- coding: utf-8 -*-
"""
Generate heatmaps to visualize edit positions and frequencies in cell barcode and UMI
...
try:
	...
	from scat_module import heatmap
except:
	import scbam
	import heatmap
...
def main():
	...
	scbam.barcode_info(infile = options.in_file, outfile = options.out_file, limit = options.reads_num)

	CB_mat_file = options.out_file + '.CB_edits_count.csv'
	CB_heatmap_file = options.out_file + '.CB_edits_heatmap'
	heatmap.make_heatmap(infile = CB_mat_file, outfile = CB_heatmap_file, filetype=options.file_type, cell_width=options.cell_width, cell_height=options.cell_height, col_angle=options.col_angle, font_size=options.font_size, text_color=options.text_color, display_num = True, log2_scale=True)

	UMI_mat_file = options.out_file + '.UMI_edits_count.csv'
	UMI_heatmap_file = options.out_file + '.UMI_edits_heatmap'
	heatmap.make_heatmap(infile = UMI_mat_file, outfile = UMI_heatmap_file, filetype=options.file_type, cell_width=options.cell_width, cell_height=options.cell_height, col_angle=options.col_angle, font_size=options.font_size, text_color=options.text_color, display_num = True, log2_scale=True)
