...
def CBC_UMIcount(infile, outfile, step_size=10000, CB_tag = 'CB', UMI_tag = 'UB', CB_num = 100000, read_num=200):
	...
	print ('\t'.join(['Cell_barcode', 'read_count', 'UMI_count']), file=OUT)
	count = 0
	...
	"""
	ROUT = open(outfile + '.Read_freq.r','w')
	logging.info ("Writing R script to \"%s\"" % (outfile + '.Read_freq.r'))
	print ("pdf(\"%s\",width=9,height=7)" % (outfile + '.Read_freq.pdf'), file=ROUT)
	print ("x <- c(%s)" % ','.join([str(i) for i in thresholds]), file = ROUT)
	print ("x.lab <- c(%s)" % ','.join([('"' + '>' + str(i) + '"') for i in thresholds]), file = ROUT)
	print ("y <- c(%s)" % ','.join([str(table[i]) for i in thresholds]), file = ROUT)
	print ('plot(x,y,type="b",xaxt = "n", xlab="Cutoff (read count)", ylab="Number of cells", col="blue")', file=ROUT)
	print ('axis(1, at=x, labels=x.lab)', file=ROUT)
	print ('text(x,y,labels=y,pos=3,col="red",cex=0.7)', file=ROUT)
	ROUT.close()
	...
	table = {}
	tmp = CB_UMI_freq.values()
	upper_bound = upper_bound
	step = step
	thresholds = list(range(0, upper_bound + step,step))[1:]
	for t in thresholds:
		if max(tmp) < t:
			table[t] = 0
		else:
			table[t] = len([i for i in tmp if i >= t])

	OUT = open(outfile + '.UMI_freq.tsv','w')
	logging.info ("Writing cell barcodes' reads frequencies to \"%s\"" % (outfile + '.UMI_freq.tsv'))
	print ('Cell_barcode' + '\t' + 'number_of_UMIs', file=OUT)
	for k,v in CB_UMI_freq.items():
		print (k + '\t' + str(v), file=OUT)
	OUT.close()
	CB_UMI_freq.clear()

	ROUT = open(outfile + '.UMI_freq.r','w')
	logging.info ("Writing R script to \"%s\"" % (outfile + '.UMI_freq.r'))
	print ("pdf(\"%s\",width=9,height=7)" % (outfile + '.UMI_freq.pdf'), file=ROUT)
	print ("x <- c(%s)" % ','.join([str(i) for i in thresholds]), file = ROUT)
	print ("x.lab <- c(%s)" % ','.join([('"' + '>' + str(i) + '"') for i in thresholds]), file = ROUT)
	print ("y <- c(%s)" % ','.join([str(table[i]) for i in thresholds]), file = ROUT)
	print ('plot(x,y,type="b",xaxt = "n", xlab="Cutoff (read count)", ylab="Number of cells", col="blue")', file=ROUT)
	...
	try:
		subprocess.call("Rscript " + (outfile + '.Read_freq.r'),shell=True)
	except:
		logging.error("Cannot generate pdf file from " + (outfile + '.Read_freq.r'))

	try:
		subprocess.call("Rscript " + (outfile + '.UMI_freq.r'),shell=True)
	except:
		logging.error("Cannot generate pdf file from " + (outfile + '.UMI_freq.r'))
	"""
